            ...
            steps {
                ...
                container('flyway') {
                    script {
                        ...
                        if (env.BRANCH_NAME == 'dev' ) {
                            sh 'flyway info -url="jdbc:mysql://${db_host_dev}:3306/${db_db}" -user=${db_user} -password=${db_pass}'
                            sh 'flyway migrate -locations=filesystem:scripts -url="jdbc:mysql://${db_host_dev}:3306/${db_db}" -user=${db_user} -password=${db_pass}'
                            sh 'flyway info -url="jdbc:mysql://${db_host_dev}:3306/${db_db}" -user=${db_user} -password=${db_pass}'
                        }
                    }
                }
